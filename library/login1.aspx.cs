...
namespace library
{
    public partial class WebForm1 : System.Web.UI.Page
    {
        ...
        protected void Login1_Authenticate(object sender, AuthenticateEventArgs e)
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand();
            cmd.CommandText = "select l_pwd,l_role from management where l_name='" + Login1.UserName + "'";
            cmd.Connection = con;
            SqlDataReader rdr = cmd.ExecuteReader();
            rdr.Read();
            try
            {
                string correctpass = rdr["l_pwd"].ToString();
                string role = rdr["l_role"].ToString();
                if (Login1.Password == correctpass)
                {
                    Session["u_name"] = Login1.UserName;
